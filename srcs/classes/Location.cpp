...
Location::Location(VirtualServer* vs)
...
bool Location::initUri(std::istringstream& iss) {
...
bool Location::parseLocationContent(std::istream& config) {
	...
	for (; std::getline(config, line);) {
		...
		if (keyword == "}") {
			...
			try {
				...
				std::cerr << CONFIG_FILE_ERROR << "Invalid keyword in location block: " << keyword
						  << std::endl;
				return false;
			}
		}
...
void Location::printLocationInformation() const {
	...
	for (std::vector<std::string>::const_iterator it = _indexPages.begin(); it != _indexPages.end();
		 it++)
		std::cout << *it << ", ";
	std::cout << std::endl;
}

void Location::initKeywordMap() {
...
bool Location::parseRoot(std::istringstream& iss) {
	std::string value;
	if (!(iss >> value)) {
...
bool Location::parseAutoIndex(std::istringstream& iss) {
	std::string value;
	if (!(iss >> value)) {
...
bool Location::parseClientBodyBufferSize(std::istringstream& iss) {
	std::string value;
	if (!(iss >> value)) {
		std::cerr << CONFIG_FILE_ERROR
...
bool Location::parseClientMaxBodySize(std::istringstream& iss) {
	std::string value;
	if (!(iss >> value)) {
		std::cerr << CONFIG_FILE_ERROR
				  << "Missing information after client_body_buffer_size keyword" << std::endl;
...
bool Location::parseErrorPages(std::istringstream& iss) {
	std::string code;
	std::string tmpStr;
...
bool Location::parseErrorCode(std::string& code, std::vector<int>& codeList) {
	int codeValue;
...
bool Location::parseIndex(std::istringstream& iss) {
	std::string value;
	if (!(iss >> value)) {
...
void Location::checkIndexPages() {
	if (_indexPages.empty()) {
		std::vector<std::string> indexPages = _associatedServer->getIndexPages();
...
void Location::checkErrorPages() {
	std::map<int, std::string> errorPages = _associatedServer->getErrorPages();
	for (std::map<int, std::string>::iterator it = errorPages.begin(); it != errorPages.end();
		 it++) {
