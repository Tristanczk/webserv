...
bool VirtualServer::initServer(std::istream& config) {
...
void VirtualServer::printServerInformation() {
...
std::string VirtualServer::getRootDir() const { return _rootDir; }

bool VirtualServer::getAutoIndex() const { return _autoIndex; }

std::size_t VirtualServer::getBufferSize() const { return _bufferSize; }

std::size_t VirtualServer::getBodySize() const { return _bodySize; }

std::map<int, std::string> VirtualServer::getErrorPages() const { return _errorPages; }

std::vector<std::string> VirtualServer::getIndexPages() const { return _indexPages; }

void VirtualServer::initKeywordMap() {
...
bool VirtualServer::parseListen(std::istringstream& iss) {
	...
	if (idx == std::string::npos) {
		...
		if (!(hostPort >> host >> port)) {
			std::cerr << CONFIG_FILE_ERROR << "Invalid format for host:port in listen instruction"
					  << std::endl;
			return false;
		}
		if (!getIPvalue(host, _address.sin_addr.s_addr)) {
			std::cerr << CONFIG_FILE_ERROR << "Invalid IPv4 address format in listen instruction"
					  << std::endl;
			return false;
		}
		if (port > 65535) {
...
bool VirtualServer::parseServerNames(std::istringstream& iss) {
	std::string value;
	if (!(iss >> value)) {
...
bool VirtualServer::parseRoot(std::istringstream& iss) {
	std::string value;
	if (!(iss >> value)) {
...
bool VirtualServer::parseAutoIndex(std::istringstream& iss) {
	std::string value;
	if (!(iss >> value)) {
...
bool VirtualServer::parseClientBodyBufferSize(std::istringstream& iss) {
	std::string value;
	if (!(iss >> value)) {
		std::cerr << CONFIG_FILE_ERROR
	...
	if (iss >> value) {
		std::cerr << CONFIG_FILE_ERROR << "Too many arguments after client_body_buffer_size keyword"
				  << std::endl;
		return false;
	}
	return true;
...
bool VirtualServer::parseClientMaxBodySize(std::istringstream& iss) {
	std::string value;
	if (!(iss >> value)) {
		std::cerr << CONFIG_FILE_ERROR
				  << "Missing information after client_body_buffer_size keyword" << std::endl;
	...
	if (iss >> value) {
		std::cerr << CONFIG_FILE_ERROR << "Too many arguments after client_max_body_size keyword"
				  << std::endl;
		return false;
	}
	return true;
...
bool VirtualServer::parseErrorPages(std::istringstream& iss) {
	std::string code;
	std::string tmpStr;
...
bool VirtualServer::parseErrorCode(std::string& code, std::vector<int>& codeList) {
	int codeValue;
...
bool VirtualServer::parseIndex(std::istringstream& iss) {
	std::string value;
	if (!(iss >> value)) {
